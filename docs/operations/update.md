...
curl -s http://localhost:8000/api/v1/system/version | python3 -m json.tool
...
### verify
- run: `docker compose -f ${INSTALL_DIR:-/opt/csgateway}/infra/docker-compose.dev.yml run --rm backend bash -c "cd /app && alembic check" 2>&1` output contains "up to date"
...
```bash
cd ${INSTALL_DIR:-/opt/csgateway}
docker compose -f infra/docker-compose.dev.yml run --rm backend bash -c "cd /app && alembic downgrade -1"
```
...
docker compose -f infra/docker-compose.dev.yml up -d --force-recreate backend frontend nginx
...
if [ -n "$TOKEN" ]; then
    echo "  Auth:   ✓"
    MODELS=$(curl -sf http://localhost:8000/api/v1/models \
...
### verify
- run: `curl -sf http://localhost:8000/api/v1/health/detailed | python3 -c "import sys,json; d=json.load(sys.stdin)['data']['checks']; print(d['database'] and d['redis'])"` output is "True"
